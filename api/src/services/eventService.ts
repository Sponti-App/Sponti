...
export const getEventById = async (userId: string, eventId: string) => {
  const baseFilter = await buildAccessibleEventFilter(userId);
  const event = await Event.findOne(withConditions(baseFilter, { _id: toObjectId(eventId) })).lean();
...
const fetchHostProfiles = async (hostIds: string[]) => {
  if (hostIds.length === 0) {
    return new Map<string, { _id: string; username?: string; displayName?: string; avatarUrl?: string | null }>();
  }
...
const attachHostProfiles = async <T extends { hostId: unknown }>(
  events: T[]
): Promise<Array<T & { hostId: { _id: string; username?: string; displayName?: string; avatarUrl?: string | null } | string | null }>> => {
  if (events.length === 0) {
    return [];
  }
